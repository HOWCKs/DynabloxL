...
class MenuHubService : Service() {
    ...
    /** Style resource the current window was built against; see [SkeuoTheme.signature]. */
    private var themeSignature: Int = 0
    ...
    private fun show(openPanel: Boolean) {
        ...
        hubView = view
        themeSignature = SkeuoTheme.signature(this, container.settings)
    ...
    private fun observe() {
        ...
        observeJob = scope.launch {
            container.settings.changes.collect {
                val view = hubView ?: return@collect
                // Theme/finish swaps replace the style resource behind the view's themed context,
                // which a ContextThemeWrapper cannot adopt after construction: rebuild the window,
                // preserving whether the panel was open.
                val signature = SkeuoTheme.signature(this@MenuHubService, container.settings)
                if (signature != themeSignature) {
                    themeSignature = signature
                    val wasOpen = view.state == MenuHubView.State.PANEL
                    teardown()
                    show(openPanel = wasOpen)
                    return@collect
                }
                view.refreshTheme()
                reposition(animate = false)
