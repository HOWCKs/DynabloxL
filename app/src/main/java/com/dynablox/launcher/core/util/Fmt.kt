...
object Fmt {
    ...
    /** Plain number for LCD readouts — the unit is drawn by the widget, not baked into the text. */
    fun number(value: Float, decimals: Int = 0): String = "%.${decimals}f".format(value)
    ...
    /**
     * Compact log scale helper for bar meters: 0 at [min], 1 at [max], logarithmic in between.
     *
     * A logarithmic scale is undefined for min <= 0 (and useless when min == max), so those cases
     * fall back to linear normalisation instead of returning NaN — a NaN would silently poison
     * every gauge that consumes it.
     */
    fun logScale(value: Float, min: Float, max: Float): Float {
        if (value <= min) return 0f
        if (value >= max) return 1f
        if (max <= min) return 0f
        if (min <= 0f) return ((value - min) / (max - min)).coerceIn(0f, 1f)
