...
abstract class DbxActivity : AppCompatActivity(), CommandFeedback {
    ...
    /**
     * Everything that cannot be re-tinted in place. When any of it changes the activity is
     * recreated, because the themed context and the inflated view tree were built against it.
     * The surface finish belongs here: it swaps the style resource, not just the token values.
     */
    private fun signature(): String = listOf(
        settings.surfaceFinish, settings.themeMode, settings.densityScale,
        settings.translucentSurfaces, settings.shadowIntensity, settings.reduceMotion,
