...
class SettingsActivity : DbxActivity() {
    ...
    private fun buildAppearance() {
        ...
        card {
            // Surface comes first: it is the most consequential visual decision in the product,
            // and it changes the palette, so the theme control below reads against the new one.
            segmentedRow(
                R.string.settings_finish,
                listOf(
                    SegmentOption(
                        getString(R.string.settings_finish_clay),
                        { settings.surfaceFinish == AppSettings.SurfaceFinish.CLAY },
                    ) {
                        settings.surfaceFinish = AppSettings.SurfaceFinish.CLAY
                        recreate()
                    },
                    SegmentOption(
                        getString(R.string.settings_finish_machined),
                        { settings.surfaceFinish == AppSettings.SurfaceFinish.MACHINED },
                    ) {
                        settings.surfaceFinish = AppSettings.SurfaceFinish.MACHINED
                        recreate()
                    },
            ...
            paragraph(R.string.settings_finish_desc)
            segmentedRow(
