    ...
    override fun onDraw(canvas: Canvas) {
        ...
        val knurl = Paint(Paint.ANTI_ALIAS_FLAG).apply {
            shader = android.graphics.BitmapShader(
                Textures.knurl(), android.graphics.Shader.TileMode.REPEAT, android.graphics.Shader.TileMode.REPEAT,
            )
            alpha = 90
        }
        val save = canvas.save()
        canvas.clipPath(Skeuo.roundRectPath(SkeuoPathHolder.path, thumb, thumbRadius))
        canvas.drawRect(thumb, knurl)
        canvas.restoreToCount(save)

        Skeuo.drawBevel(
            canvas, thumb, thumbRadius,
            Tint.alphaFraction(tokens.chromeHi, 0.55f),
            Tint.alphaFraction(Color.BLACK, 0.6f),
            1f, density,
        )
        // grip ridge
...
class RotaryKnob @JvmOverloads constructor(
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        paint.shader = null
        Skeuo.drawBevel(
            canvas, top, topRadius,
            Tint.alphaFraction(tokens.chromeHi, 0.6f), Tint.alphaFraction(Color.BLACK, 0.6f),
            1.2f, density,
        )
...
class FaderSlider @JvmOverloads constructor(
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        val knurl = Paint(Paint.ANTI_ALIAS_FLAG).apply {
            shader = android.graphics.BitmapShader(
                Textures.knurl(height = 24), android.graphics.Shader.TileMode.REPEAT, android.graphics.Shader.TileMode.REPEAT,
            )
            alpha = 80
        }
        val save = canvas.save()
        canvas.clipPath(Skeuo.roundRectPath(SkeuoPathHolder.path, cap, capRadius))
        canvas.drawRect(cap, knurl)
        canvas.restoreToCount(save)
        Skeuo.drawBevel(
            canvas, cap, capRadius,
            Tint.alphaFraction(tokens.chromeHi, 0.6f), Tint.alphaFraction(Color.BLACK, 0.6f),
            1f, density,
        )
        // centre groove
