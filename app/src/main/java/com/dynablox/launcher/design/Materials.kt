...
/**
 * The two material vocabularies the product can wear.
 *
 * They share geometry, tokens, spacing and components — only the physics of the surface change,
 * which is what makes the migration reversible and testable.
 */
enum class Finish {
    /** Machined instrument: brushed metal, chrome chamfers, hard specular. */
    MACHINED,

    /** CLAY MORPHIST: pressed matter, diffuse light, thick shoulders, no gloss. */
    CLAY,
}
...
object Materials {
    ...
    /**
     * Single entry point for every surface in the product.
     *
     * The active [Finish] decides which physics paint the form. Call sites never branch on it:
     * migrating the whole product between finishes is one preference, not a refactor.
     */
    fun paint(
        canvas: Canvas,
        box: RectF,
        ...
        when (SkeuoTheme.finish) {
            Finish.MACHINED -> paintMachined(canvas, box, radius, material, tokens, state, options, density)
            Finish.CLAY -> Clay.paint(canvas, box, radius, material, tokens, state, options, density)
        }
    }

    /**
     * Corner radius the active finish wants for a given requested radius. Clay cannot hold a tight
     * corner, so widgets that clip or stroke their own geometry ask here instead of assuming.
     */
    fun radiusFor(box: RectF, radius: Float): Float = when (SkeuoTheme.finish) {
        Finish.MACHINED -> radius
        Finish.CLAY -> Clay.clayRadius(box, radius)
    }

    /**
     * Rim treatment for circular instruments, resolved per finish: a rotating chrome sweep when
     * machined, a soft directional band when clay. Widgets ask for "the rim", not for chrome.
     */
    fun rimShader(cx: Float, cy: Float, radius: Float, tokens: ThemeTokens): android.graphics.Shader =
        when (SkeuoTheme.finish) {
            Finish.MACHINED -> Skeuo.chromeSweep(
                cx, cy, radius, tokens.chromeHi, Tint.darken(tokens.chromeLo, 0.2f),
            )
            Finish.CLAY -> Clay.rimShader(cx, cy, radius, tokens)
        }

    /**
     * Edge treatment for a pressed/raised part. Machined parts get a hard 1 px chamfer; clay parts
     * get nothing here, because [Clay.paint] already drew their shoulders — stroking again would
     * put a machined line on a soft form.
     */
    fun drawPartEdge(
        canvas: Canvas,
        box: RectF,
        ...
        if (SkeuoTheme.finish != Finish.MACHINED) return
        Skeuo.drawBevel(
            canvas, box, radius,
            Tint.alphaFraction(tokens.chromeHi, 0.55f),
            Tint.alphaFraction(android.graphics.Color.BLACK, 0.6f),
            1f, density,
        )
    }

    /**
     * Grip marking on a movable part: knurling when machined, a thumb dimple when clay.
     * Both say "hold here"; only one of them is physically possible in each vocabulary.
     */
    fun drawGrip(
        canvas: Canvas,
        box: RectF,
        ...
        when (SkeuoTheme.finish) {
            Finish.MACHINED -> {
                ...
                canvas.clipPath(Skeuo.roundRectPath(SkeuoPathHolder.path, box, radius))
                canvas.drawRect(box, knurl)
                canvas.restoreToCount(save)
            }
            Finish.CLAY -> Clay.drawGripDimple(canvas, box, tokens, density)
        }
    }

    private fun paintMachined(
        canvas: Canvas,
        box: RectF,
        radius: Float,
        material: Material,
